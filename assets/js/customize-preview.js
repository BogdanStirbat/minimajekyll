/* global minimaJekyllBgColors, minimaJekyllPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( minimaJekyllBgColors ).forEach( function( context ) {
					minimaJekyllGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function minimaJekyllGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'minimajekyll-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#minimajekyll-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( minimaJekyllPreviewEls[ context ] ) {
				_.each( minimaJekyllPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		minimaJekyllGenerateColorA11yPreviewStyles( 'content' );
		minimaJekyllGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
