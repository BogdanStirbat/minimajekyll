/* global minimaJekyllBgColors, minimaJekyllColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( minimaJekyllBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( minimaJekyllBgColors[ context ].color ) {
						backgroundColorValue = minimaJekyllBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( minimaJekyllBgColors[ context ].setting ).get();
					}
					minimaJekyllSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( minimaJekyllBgColors ).forEach( function( context ) {
			wp.customize( minimaJekyllBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					minimaJekyllSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		minimaJekyllSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				minimaJekyllSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function minimaJekyllSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = minimaJekyllColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Minima Jekyll 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function minimaJekyllSetRetineLogoVisibility( visible ) {
